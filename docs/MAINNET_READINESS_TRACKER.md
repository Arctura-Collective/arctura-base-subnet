...
| #4 AWS EC2 Auto-Scaling Miners & Dynamic Validator Health Checks | Terraform ASG/CloudWatch artifacts and tests exist under `deploy/aws/asg/`. | AWS AMI, subnet, security group, instance profile, CloudWatch metric publication, and production apply are not provisioned from this repo. | Operator-approved `terraform plan`, production apply evidence, ASG in-service capacity, and CloudWatch alarm delivery evidence. |
